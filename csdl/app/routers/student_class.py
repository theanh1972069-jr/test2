...
@router.get("/{student_id}/{class_id}/{subject_id}", response_model=StudentClassInDB)
def read_student_class(student_id: int, class_id: int, subject_id: int, db: Session = Depends(get_db)):
